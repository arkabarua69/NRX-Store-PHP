...
class StatsOverviewWidget extends BaseWidget
{
    ...
    private function calculateRevenueIncreaseByDate(): array
    {
        $currentRevenue = Order::whereMonth('created_at', now()->month)
            ...
            ->sum('amount');
        $previousRevenue = Order::whereMonth('created_at', now()->subMonth()->month)
    ...
    {
        $currentOrders = Order::whereMonth('created_at', now()->month)
            ->whereYear('created_at', now()->year)
            ->count();
        $previousOrders = Order::whereMonth('created_at', now()->subMonth()->month)
            ->whereYear('created_at', now()->subMonth()->year)
            ->count();
    ...
    private function calculateUserIncreaseByDate(): array
    {
        $currentUsers = User::whereMonth('created_at', now()->month)
            ...
            ->count();
        $previousUsers = User::whereMonth('created_at', now()->subMonth()->month)
            ->whereYear('created_at', now()->subMonth()->year)
            ->where('role', Role::USER)
            ->count();
